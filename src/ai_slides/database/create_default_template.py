...
def get_template_examples_path() -> Path:
    ...
    # 从 src/ai_slides/database/create_default_template.py 向上四级到项目根目录
...
def load_template_from_json(json_file_path: Path) -> Dict[str, Any]:
    ...
    Returns:
        Dictionary containing template data, or None if loading fails
    """
    try:
        if not json_file_path.exists():
            logger.error(f"Template file does not exist: {json_file_path}")
            return None
        ...
        if 'template_name' not in template_data:
            logger.error(f"Missing 'template_name' in template file: {json_file_path}")
            return None
            
        if 'html_template' not in template_data:
            logger.error(f"Missing 'html_template' in template file: {json_file_path}")
            return None
        ...
        logger.debug(f"Successfully loaded template '{template_data['template_name']}' from {json_file_path.name}")
        return template_data

    except json.JSONDecodeError as e:
        logger.error(f"Invalid JSON in template file {json_file_path}: {e}")
        return None
...
async def import_templates_from_examples() -> List[int]:
    ...
    try:
        async with AsyncSessionLocal() as session:
            ...
            if len(json_files) == 0:
                logger.warning("No template files found in template_examples directory")
                return []
            ...
            for json_file in json_files:
                ...
                if not template_data:
                    logger.warning(f"Skipping invalid template file: {json_file.name}")
                    continue
                ...
                if existing:
                    ...
                    imported_template_ids.append(existing.id)
                    # Track if this is the default template
                    if template_data['template_name'] == "默认商务模板":
                        default_template_id = existing.id
                    continue

                # 创建模板
                try:
                    # Set 商务 template as default
                    if template_data['template_name'] == "默认商务模板":
            ...
            if len(imported_template_ids) != 4:
                logger.warning(f"Expected 4 templates but found {len(imported_template_ids)}")
            
            # Ensure default template is set
            if default_template_id:
                logger.info(f"Default template (商务) set with ID: {default_template_id}")
            else:
                logger.warning("Default template (商务) not found in imported templates")
...
async def ensure_default_templates_exist(force_import: bool = False):
    ...
    try:
        ...
        if not force_import:
            async with AsyncSessionLocal() as session:
                ...
                if existing_templates:
                    ...
                    # Verify we have exactly 4 templates
                    if len(existing_templates) != 4:
                        logger.warning(f"Expected 4 templates but found {len(existing_templates)}")
                    
                    # Verify default template exists
                    default_template = await db_service.get_default_global_master_template()
                    if not default_template:
                        logger.warning("No default template found, attempting to set one")
                        # Try to find and set 商务 template as default
                        for template in existing_templates:
                            if template.template_name == "默认商务模板":
                                await db_service.update_global_master_template(template.id, {"is_default": True})
                                logger.info(f"Set template '{template.template_name}' (ID: {template.id}) as default")
                                break
        ...
        if imported_ids:
            ...
            logger.warning("No templates imported from examples, creating fallback default template")
            template_id = await create_default_global_template()
            if template_id:
                logger.info(f"Fallback default template created with ID: {template_id}")
                return [template_id]
            else:
                logger.error("Failed to create fallback default template")
                return []
