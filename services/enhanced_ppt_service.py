...
<body>
    ...
    def _combine_slides_to_full_html(self, slides_data: List[Dict[str, Any]], title: str) -> str:
        ...
        try:
            ...
            temp_dir = Path(tempfile.gettempdir()) / "ai_slides" / presentation_id
...
<body>
    ...
    def _standardize_summeryfile_outline(self, summeryfile_outline: Dict[str, Any]) -> Dict[str, Any]:
        """
        将summeryanyfile生成的大纲格式标准化为AI Slides格式
    ...
    async def conduct_research_and_merge_with_files(
        ...
        try:
            ...
            # 创建临时文件
            temp_dir = Path(tempfile.gettempdir()) / "ai_slides_merged"
    ...
    async def generate_outline_from_file(self, request) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                # 转换为AI Slides格式 - 使用新的标准化函数
                summeryfile_dict = outline.to_dict()
                ai_slides_outline = self._standardize_summeryfile_outline(summeryfile_dict)

                # 验证和修复文件生成的大纲
                # 构建confirmed_requirements用于验证
                confirmed_requirements = {
                    'topic': request.topic or ai_slides_outline.get('title', '文档演示'),
                ...
                ai_slides_outline = await self._validate_and_repair_outline_json(ai_slides_outline, confirmed_requirements)
    ...
    def _convert_summeryfile_outline_to_ai_slides(self, summery_outline, request) -> Dict[str, Any]:
        """将summeryanyfile的大纲格式转换为AI Slides格式"""
        try:
            slides = []
    ...
    async def _generate_outline_from_file_fallback(self, request):
        ...
        # 创建基于文件内容的PPT大纲
        ai_slides_outline = self._create_outline_from_file_content(content, request)
        ...
        # 获取处理统计信息
        slides_count = len(ai_slides_outline.get('slides', []))
        processing_stats = {
        ...
        logger.info(f"简化版本大纲生成成功: {ai_slides_outline.get('title', '未知')}, 共{slides_count}页")
